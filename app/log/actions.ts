  ...
  try {
    ...
    await sql`
      ...
      ON CONFLICT (date) DO UPDATE SET
        sleep_duration     = COALESCE(EXCLUDED.sleep_duration,  entries.sleep_duration),
        bed_time           = COALESCE(EXCLUDED.bed_time,        entries.bed_time),
        wake_time          = COALESCE(EXCLUDED.wake_time,       entries.wake_time),
        hrv                = COALESCE(EXCLUDED.hrv,             entries.hrv),
        deep_min           = COALESCE(EXCLUDED.deep_min,        entries.deep_min),
        core_min           = COALESCE(EXCLUDED.core_min,        entries.core_min),
        rem_min            = COALESCE(EXCLUDED.rem_min,         entries.rem_min),
        awake_min          = COALESCE(EXCLUDED.awake_min,       entries.awake_min),
        deep_pct           = COALESCE(EXCLUDED.deep_pct,        entries.deep_pct),
        core_pct           = COALESCE(EXCLUDED.core_pct,        entries.core_pct),
        rem_pct            = COALESCE(EXCLUDED.rem_pct,         entries.rem_pct),
        mood               = COALESCE(EXCLUDED.mood,            entries.mood),
        focus              = COALESCE(EXCLUDED.focus,           entries.focus),
        water_oz           = COALESCE(EXCLUDED.water_oz,        entries.water_oz),
        alcohol_count      = COALESCE(EXCLUDED.alcohol_count,   entries.alcohol_count),
        alcohol_desc       = COALESCE(EXCLUDED.alcohol_desc,    entries.alcohol_desc),
        coffee_count       = COALESCE(EXCLUDED.coffee_count,    entries.coffee_count),
        breakfast_notes    = COALESCE(EXCLUDED.breakfast_notes, entries.breakfast_notes),
        lunch_notes        = COALESCE(EXCLUDED.lunch_notes,     entries.lunch_notes),
        dinner_notes       = COALESCE(EXCLUDED.dinner_notes,    entries.dinner_notes),
        snack_notes        = COALESCE(EXCLUDED.snack_notes,     entries.snack_notes),
        general_notes      = COALESCE(EXCLUDED.general_notes,   entries.general_notes),
